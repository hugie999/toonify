...
def loadAsB64(url:str) -> str:
    if imageCache.get(url,None):
        IMG = imageCache.get(url,None)
    else:
        IMG = b64.encodebytes(wtp.loadImage(url)).decode('ascii').replace("\n","")
    imageCache.update({url:IMG})
    return IMG
...
def readPage(pg:fletnav.PageData):
    # print(pg.arguments)
    pg.page.client_storage.set("ca.hugie999.toonify.imgCache",imageCache)
    ...
    def search(e):
        ...
        if typePicker.value == "canvas":
            ...
            if len(comics) == 0:
                ...
                print("no results!")
                cont = ft.Container(ft.Text("no results found!"),padding=ft.Padding(10,10,10,10),height = 0,animate_size=ft.Animation(600,ft.AnimationCurve.EASE_OUT_CIRC))
...
def settingsPage(pg:fletnav.PageData):
    ...
    cacheSlider = ft.Slider(min=0,max=100,divisions=5,label="{value}")
    pg.add(
        ft.Card(ft.Container(
            ft.Column([ft.Text("image caching amount:"),cacheSlider])
            ,padding=ft.Padding(10,10,10,10)),elevation=20)
        )
    pg.add(ft.FilledButton("reset cache",
                        icon=ft.icons.RESTORE,
                        icon_color=ft.colors.RED,
                        on_click=lambda _: pg.page.client_storage.remove("ca.hugie999.toonify.storedcache")))
    pg.add(ft.Container(
...
def main(page: ft.Page):
    ...
    if TOKEN:
        ...
        if page.client_storage.contains_key("ca.hugie999.toonify.imgCache"):
            imageCache = page.client_storage.get("ca.hugie999.toonify.imgCache")
        else:
            page.client_storage.set("ca.hugie999.toonify.imgCache",{})
    ...
    if page.client_storage.contains_key("ca.hugie999.toonify.storedcache"):
        ...
        saveCache(page)
    print("chache EVERYTHING...")
    # if not (webtoonapi.cache.listedOriginals):
    #     page.appbar = ft.AppBar(leading=ft.Text("please wait..."),elevation=50)
    # if not webtoonapi.cache.listedCanvas:
    #     print("canvas...")
    #     webtoonapi.listComics("canvas")
    if not webtoonapi.cache.listedOriginals:
        print("originals...")
        page.appbar = ft.AppBar(leading=ft.Text("please wait..."),elevation=50)
        page.update()
        webtoonapi.listComics("originals")
    page.appbar = None
    print(type(webtoonapi))
